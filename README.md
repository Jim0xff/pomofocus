...
1. **Install dependencies**
   ...
   npm install
   ```
2. **Create `.env`** (sample values in `.env.example`). 主要变量包括 `DATABASE_URL`、`REDIS_URL`、`JWT_SECRET`、`TASK_POINT_URL`（生产环境指向 task-point 鉴权服务），以及 `TEST_AUTH`（dev/staging 可设为 true 并传入任意 ETH 地址作为 token）。
